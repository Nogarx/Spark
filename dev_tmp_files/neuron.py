...
@register_config
class NeuronConfig(BaseSparkConfig):
	"""
	...
	def _validate_maps(self,):
		"""
			Basic validation of the configuration maps to ensure that neuron can properly read the data.
		"""
...
class Neuron(SparkModule, metaclass=NeuronMeta):
	"""
		Neuron model.
	...
	# TODO: I think most of this validation can be moved to NeuronConfig
	...
	def get_spikes_from_cache(self) -> dict:
		"""
			Collect the neuron's spikes.
		"""
		neuron_spikes = {}
		for module_name in self._modules_list:
			if 'spikes' in self._cache[module_name]:
				neuron_spikes[module_name] = self._cache[module_name]['spikes'].value
			elif 'out_spikes' in self._cache[module_name]:
				neuron_spikes[module_name] = self._cache[module_name]['out_spikes'].value
